...
file_U8II_thrust = os.path.join(
    ...
    "AlphaESCTelemetry",
    "data",
    # "2023-02-23T14-33-30_AlphaTelemetry_thrust.csv",
    # "2023-02-24T13-55-09_AlphaTelemetry_faultyESC_longThrust.csv",
    "2023-02-23T15-04-29_AlphaTelemetry_phase_fault.csv",
)
...
def read_file(file) -> pandas.DataFrame:
    ...
    # Remove rows where current is greater than 500A
    df = df[df["busbarCurrent"] < 500]
    ...
    voltage_scaler = 1 + (numpy.clip(df["voltage_filtered"], 3.5 * 6, 4.2 * 6) - 4.2 * 6) / (3.5 * 6)
    df["voltage_scaler"] = voltage_scaler
...
plt.subplot(2, 1, 1)
plt.plot(df["voltage_scaler"], label="voltage_scaler")
plt.legend()
plt.subplot(2, 1, 2)
plt.plot(df["phaseWireCurrent"], label="phaseWireCurrent")
plt.plot(df["busbarCurrent"], label="busbarCurrent")
plt.plot(df["phaseWireCurrent"] / df["voltage_scaler"], label="phaseWireCurrent/voltage_scaler")
plt.plot(df["busbarCurrent"] / df["voltage_scaler"], label="busbarCurrent/voltage_scaler")
plt.legend()
plt.grid()
plt.show()
